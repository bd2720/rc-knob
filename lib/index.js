...
const reduceOnCancel = (state, action, callbacks) => {
    ...
    return Object.assign(Object.assign({}, state), { isActive: false, value, percentage, startPercentage: undefined, startValue: undefined });
};
...
var useUpdate = ({ min, max, multiRotation, initialValue, angleOffset = 0, angleRange = 360, size, steps, onChange, onInteractiveChange, interactiveHook, onStart, onEnd, readOnly, tracking, useMouseWheel, }) => {
    ...
    React.useEffect(handleEventListener({ container, dispatch, readOnly, useMouseWheel, interactiveHook }), [useMouseWheel, readOnly]);
    return {
        svg,
        container,
...
const isInternalComponent = ({ type }) => type === Arc || type === Pointer || type === Scale || type === Value || type === Range || type === Spiral || type === Label;
