...
const reducer = (callbacks) => (state, action) => {
    ...
    switch (action.type) {
        ...
        case 'SET_VALUE': // Handle external value updates
            return Object.assign(Object.assign({}, state), { value: (_a = action.value) !== null && _a !== void 0 ? _a : null, percentage: (_b = action.percentage) !== null && _b !== void 0 ? _b : null });
        default:
            return Object.assign(Object.assign({}, state), { isActive: false, value: state.value });
    }
};
var useUpdate = ({ min, max, multiRotation, initialValue, value, angleOffset = 0, angleRange = 360, size, steps, onChange, onInteractiveChange, interactiveHook, onStart, onEnd, readOnly, tracking, useMouseWheel, }) => {
    ...
    const [{ percentage, value: internalValue }, dispatch] = React.useReducer(reducer(callbacks), {
        isActive: false,
        min,
        max,
        multiRotation,
...
const isInternalComponent = ({ type }) => type === Arc ||
    ...
    type === Spiral ||
    type === Label;
const Knob = ({ min, max, initialValue = null, value: controlledValue = null, multiRotation = false, angleOffset = 0, angleRange = 360, size, onChange = () => { }, onInteractiveChange = () => { }, interactiveHook = undefined, onStart = () => { }, onEnd = () => { }, children, steps, snap = false, tracking = true, readOnly = false, useMouseWheel = true, ariaValueText, ariaLabelledBy, className, }) => {
    ...
    return (React__default["default"].createElement("div", { ref: container, tabIndex: 0, style: { outline: 'none', width: size, height: size }, "aria-valuemax": max, "aria-valuemin": min, "aria-valuenow": currentValue || 0, "aria-valuetext": ariaValueText, "aria-labelledby": ariaLabelledBy, onKeyDown: readOnly ? undefined : onKeyDown, className: className },
        React__default["default"].createElement("svg", { width: size, height: size, ref: svg }, React__default["default"].Children.map(children, (child) => {
            ...
            return isInternalComponent(child)
                ? React__default["default"].cloneElement(child, Object.assign({ percentage,
                    size, value: currentValue, angleOffset,
