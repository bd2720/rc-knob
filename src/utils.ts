...
export const snapPosition = (position:Position, state:{angleOffset: number, angleRange: number}, steps?:number):Position => {
...
export const getValueFromPercentage = ({ min, max, percentage }: {min:number,max:number,percentage:number}) =>
    min + (max - min) * percentage

export const getPercentageFromValue = ({ min, max, value }: {min:number,max:number,value:number}) =>
    (value - min) / (max - min)
